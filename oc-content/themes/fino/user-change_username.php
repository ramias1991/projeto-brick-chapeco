...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="<?php echo str_replace('_', '-', osc_current_user_locale()); ?>">
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $('form#change-username').validate({
                    ...
                    messages: {
                        s_username: {
                            required: '<?php echo osc_esc_js(__("Username: this field is required", "fino")); ?>.'
                        }
                    },
                        ...
                        cInterval = setInterval(function(){
                            $.getJSON(
                                ...
                                function(data){
                                    clearInterval(cInterval);
                                    if(data.exists==0) {
                                        $("#available").text('<?php echo osc_esc_js(__("The username is available", "fino")); ?>');
                                    } else {
                                        $("#available").text('<?php echo osc_esc_js(__("The username is NOT available", "fino")); ?>');
                                    }
                                }
                            );
                        }, 1000);
    ...
    <body>
        ...
        <div class="content user_account">
            <h1>
                <strong><?php _e('User account manager', 'fino'); ?></strong>
            </h1>
            <div id="sidebar">
                <?php echo osc_private_user_menu( get_user_menu() ); ?>
            </div>
            <div id="main" class="modify_profile">
                <h2><?php _e('Change your username', 'fino'); ?></h2>
                <ul id="error_list"></ul>
                <form id="change-username" action="<?php echo osc_base_url(true); ?>" method="post">
                    ...
                    <fieldset>
                        <p>
                            <label for="s_username"><?php _e('Username', 'fino'); ?></label>
                            <input type="text" name="s_username" id="s_username" value="" />
                        </p>
                        <p>
                            <span class="help-box" ><?php _e('WARNING: Once set, you will not be able to change your username again. Choose wisely.', 'fino'); ?></span>
                        </p>
